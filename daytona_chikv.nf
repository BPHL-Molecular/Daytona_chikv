...
/*
  Daytona Chikungunya
  Florida's BPHL Nextflow pipeline for Chikungunya virus NGS data analysis
  Authors: Arnold Rodriguez, Molly Mitchell
...
workflow {
    ...
    log.info """
    Daytona Chikungunya - BPHL Chikungunya virus NGS pipeline
    ==========================================================================
    ...
    // CHIKV reference (FASTA + bwa index), annotation, and primer assets
    ch_ref    = channel.value(refFiles())
    ch_ref_fa = channel.value(file("${projectDir}/assets/reference/chikv.reference.fasta"))
    ch_gff    = channel.value(file("${projectDir}/assets/annotations/CHIKV_LC259094.1_genomic.gff"))
    ch_primer = channel.value(file("${projectDir}/assets/primers/CHIKV.primer.bed"))
    ...
    // Genotype assignment via Nextclade
    ...
    multiqc(
        ...
        channel.value(file("${projectDir}/assets/daytona_chikv_report.css", checkIfExists: true)),
