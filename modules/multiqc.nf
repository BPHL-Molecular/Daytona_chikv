process multiqc {
    ...
    output:
        path("daytona_chikv_report.html"), emit: report
...

    {
      ...
      grep -hoE "docker://[^']+" ${nf_config} \\
        | sed -E 's#docker://[^/]*/([^:]+):(.+)#\\1\\t\\2#' \\
        | sort -u
    } > "${params.output}/daytona_chikv_versions_mqc.tsv"

    multiqc ${params.output} \\
        -c ${multiqc_config} \\
        --filename daytona_chikv_report.html \\
        --interactive \\
        --ignore "*/multiqc/*" \\
        --ignore "*daytona_chikv_report*" \\
        --ignore "*summary_report.txt"

    rm -f "${params.output}"/daytona_chikv_*_mqc.tsv
    """
}
